...
Bu belge, **CIC-IDS2017** veri setinde yer alan **79 sütunun** (78 özellik sütunu + 1 etiket) tamamının tanımını, ait olduğu kategoriyi, veri tipini ve veri ön işleme aşamasındaki değerlendirme notlarını içerir. CSV yükleme doğrulamasında 78 özellik sütunu zorunlu ve `Label` opsiyoneldir. Model eğitimi veri hazırlama aşamasında (`prepare_training_data`) `Label` sütunu zorunludur; `Fwd Header Length.1` şema doğrulamasından sonra redundant özellik olarak düşürülür ve model girdisi deterministik sırada 77 sayısal özellikten oluşur.

## Sütun Listesi ve Tanımları

| Sıra | Sütun Adı | Veri Tipi | Kategori | Kısa Açıklama | Ön İşleme Notu |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | `Destination Port` | Integer | Flow bilgileri | Hedef port numarası | **Sayısal Özellik:** Varsayılan ön işleme pipeline'ında sayısal olarak imputer ve scaler ile işlenir (77 model özelliğinden biridir). |
...
| 56 | `Fwd Header Length.1` | Integer | Header özellikleri | Tekrarlanan ileri yön başlık uzunluğu | **Tekrarlı Sütun:** Şema doğrulamasından sonra düşürülür; model girdisine (77 özellik) dâhil edilmez. |
...
| 79 | `Label` | String | Hedef değişken | Trafik bağlantı sınıfı (Normal veya Saldırı Türü) | **Hedef Değişken:** CSV yükleme doğrulamasında opsiyoneldir; model eğitimi veri hazırlama aşamasında (`prepare_training_data`) zorunludur. |
